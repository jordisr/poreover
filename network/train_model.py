...
def build_rnn(X,y,sequence_length, ctc_merge_repeated=False):
    # model parameters
    ...
    loss = tf.reduce_mean(tf.nn.ctc_loss(labels=y, inputs=logits, sequence_length=sequence_length, time_major=False, preprocess_collapse_repeated=False, ctc_merge_repeated=bool(ctc_merge_repeated)), name='loss')
...
def train(args):
    ...
    (train_op, logits, loss, decoded, prediction, log_prob, edit_distance) = build_rnn(X,y,sequence_length,args.ctc_merge_repeated)
