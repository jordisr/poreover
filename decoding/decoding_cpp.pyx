...
cdef extern from "BeamSearch.h":
    ...
    string beam_search_2d_by_row(double**, double**, int**, int, int, string)
    string beam_search_2d_by_row(double**, double**, int, int, string)
...
@cython.boundscheck(False)
@cython.wraparound(False)
def cpp_beam_search_2d_by_row(y1_, y2_, envelope_ranges_=None, beam_width_=25, alphabet_=b'ACGT'):
    ...
    try:
        ...
        if envelope_ranges:
            for u in range(U+1):
                point_to_envelope_ranges[u] = &envelope_ranges[u,0]
            decoded_sequence = beam_search_2d_by_row(&point_to_y1[0], &point_to_y2[0], &point_to_envelope_ranges[0], U, V, alphabet.encode('UTF-8'))
        else:
            decoded_sequence = beam_search_2d_by_row(&point_to_y1[0], &point_to_y2[0], U, V, alphabet)
        return(decoded_sequence.decode("UTF-8").lstrip('\x00'))
    finally:
        free(point_to_y1)
        free(point_to_y2)
